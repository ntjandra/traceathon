...
def registerManufacturer():
    ...
    if form.validate_on_submit():
        ...
        user = Manufacturer(username=form.username.data, email=form.email.data, password=hashed_password)
        db.session.add(user)
        db.session.commit()
...
def registerSupplier():
    ...
    if form.validate_on_submit():
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        user = Supplier(username=form.username.data, email=form.email.data, password=hashed_password)
        db.session.add(user)
        db.session.commit()
        flash('Your account has been created! You are now able to log in', 'success')
        return redirect(url_for('login'))
...
def loginManufacturer():
    ...
    form = LoginForm()
    if form.validate_on_submit():
        user = Manufacturer.query.filter_by(email=form.email.data).first()
...
def loginSupplier():
    ...
    form = LoginForm()
    if form.validate_on_submit():
        user = Supplier.query.filter_by(email=form.email.data).first()
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            login_user(user, remember=form.remember.data)
            next_page = request.args.get('next')
...
def send_reset_email(user):
    ...
    msg = Message('Password Reset Request',
                    sender=os.environ.get('EMAIL_ROBOT'),
